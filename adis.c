...
void adis_raw_in(unsigned char *buffer, int len, unsigned char* timestamp) {
	if(len == sizeof(ADISMessage)){
		...
		packet.data.adis_xgyro_out = fix_complement(buffer[14] | buffer[15] << 8, 16);
		packet.data.adis_ygyro_out = fix_complement(buffer[16] | buffer[17] << 8, 16);
		packet.data.adis_zgyro_out = fix_complement(buffer[18] | buffer[19] << 8, 16);
		packet.data.adis_xaccl_out = fix_complement(buffer[20] | buffer[21] << 8, 16);
		packet.data.adis_yaccl_out = fix_complement(buffer[22] | buffer[23] << 8, 16);
		packet.data.adis_zaccl_out = fix_complement(buffer[24] | buffer[25] << 8, 16);
		packet.data.adis_xmagn_out = fix_complement(buffer[26] | buffer[27] << 8, 16);
		packet.data.adis_ymagn_out = fix_complement(buffer[28] | buffer[29] << 8, 16);
		packet.data.adis_zmagn_out = fix_complement(buffer[30] | buffer[31] << 8, 16);
		packet.data.adis_temp_out = fix_complement(buffer[32] | buffer[33] << 8, 16);
